...
// One model in one frame, and what an effect is doing to it.
struct LayerSpec {
...
struct FrameSpec {
    ...
    // Per source; screen sizes are given at 1080 lines and scaled to this frame.
    std::vector<SourceStyle> styles;
};
...
private:
    ...
    void submit_layer(int which, const LayerSpec& l);
    unsigned draw_gl_layer(Slot& s, const LayerSpec& l);
